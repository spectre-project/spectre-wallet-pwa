import {RPC} from '/@spectre/grpc-web';
...
import {isMobile} from '/@spectre/ux/spectre-ux.js';
export * from '/@spectre/ux/spectre-ux.js';

window.__testI18n = (test)=>i18n.setTesting(!!test);

class SpectreWalletHeader extends BaseElement{
	static get styles(){
		return css`
			:host{display:block}
			.container{
			...
			.logo{
				...
				background:no-repeat url('/resources/images/spectre.png') center;
...
SpectreWalletHeader.define("spectre-wallet-header")

class SpectreWalletApp extends FlowApp {
	...
	render(){
		...
		return html`
		${isMobile?'':html`<!--spectre-wallet-header></spectre-wallet-header-->`}
		<spectre-wallet .walletMeta='${meta}' reloadonlock="true" hidefaucet="true"></spectre-wallet>
		`
	}
	...
	firstUpdated(){
		...
		this.wallet = this.renderRoot.querySelector("spectre-wallet");
...
SpectreWalletApp.define("spectre-wallet-app");
